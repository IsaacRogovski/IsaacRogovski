# Hello, I'm Isaac 👋

💻 Systems Analysis and Development student at IFSP.  
🎓 IT Technician graduate from ETEC.  
🚀 Passionate about programming, web development and software engineering.

## 🛠️ Technologies: 
[![My Skills](https://skillicons.dev/icons?i=html,css,js,jquery,bootstrap,cs,php,mysql,linux,arch,git,github)](https://skillicons.dev)


## 📚 Currently Learning

- C# (focused on logic and system development)
- Arch Linux (terminal, package management, automation)
